...
use crate::helpers::youtube_api_helper::{
  youtube_delete_message_with_oauth, youtube_fetch_live_chat_id_by_api_key,
...
pub async fn youtubeFetchChatMessages(
  ...
  if live_chat_id.is_empty() {
    println!("[YouTube] No active live chat found for video: {}", video_id);
    return Ok("{\"items\": [], \"nextPageToken\": \"\", \"pollingIntervalMillis\": 2000}".to_string());
  }
...
pub async fn youtubeSendMessage(
  ...
  if access_token.is_empty() {
    return Err("Access token is required. Please connect your YouTube account in Settings.".to_string());
  }
